...
fn handle_keyboard_input(
    ...
    if let Ok(mut ship_acceleration) = ship_acceleration_query.get_single_mut() {
        if keys.just_pressed(KeyCode::Space) {ship_acceleration.value.y += FORWARD_ACCELERATION}
        if keys.just_released(KeyCode::Space) {ship_acceleration.value.y -= FORWARD_ACCELERATION}

        if keys.just_pressed(KeyCode::W) {ship_acceleration.value.y += OTHER_ACCELERATION}
        if keys.just_released(KeyCode::W) {ship_acceleration.value.y -= OTHER_ACCELERATION}

        if keys.just_pressed(KeyCode::S) {ship_acceleration.value.y -= OTHER_ACCELERATION}
        if keys.just_released(KeyCode::S) {ship_acceleration.value.y += OTHER_ACCELERATION}

        if keys.just_pressed(KeyCode::A) {ship_acceleration.value.x -= OTHER_ACCELERATION}
        if keys.just_released(KeyCode::A) {ship_acceleration.value.x += OTHER_ACCELERATION}

        if keys.just_pressed(KeyCode::D) {ship_acceleration.value.x += OTHER_ACCELERATION}
        if keys.just_released(KeyCode::D) {ship_acceleration.value.x -= OTHER_ACCELERATION}
    }
}
...
fn set_rotation(
    ...
    if let Ok((mut transform, ship)) = ship_query.get_single_mut() {
        ...
        let z = transform.rotation.mul_vec3(Vec3::Y).cross(target_direction).z;
        ...
        if angular_velocity != 0. {
            ...
            if z * transform.rotation.mul_vec3(Vec3::Y).cross(target_direction).z < 0. {
                transform.rotation = Quat::from_axis_angle(Vec3::NEG_Z, ship.target_direction.angle_between(Vec2::Y));
            }
        }
    }
}